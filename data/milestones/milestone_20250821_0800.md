...
Extraction Operations (PDFExtractor) - PDF content parsing and date verification
...
│   │   ├── pdf_extractor.py            # PDF parsing with comprehensive extraction
...
equipment (
    ...
    filter_type TEXT, filter_make TEXT, filter_model TEXT, filter_capacity_gpm INTEGER,
    pump_make TEXT, pump_model TEXT, pump_hp REAL,
    sanitizer_details TEXT,
    main_drain_details TEXT, equalizer_details TEXT,
    equipment_matches_emd BOOLEAN,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (report_id) REFERENCES inspection_reports(id),
    FOREIGN KEY (facility_id) REFERENCES facilities(id)
)
...
PDF Extraction FieldDatabase FieldMapping Notesfacility_id (from facilities)facility_idForeign keypermit_idpermit_idDirect mappinginspection_date (normalized)inspection_dateYYYY-MM-DD formatinspection_typeinspection_typeDirect mappinginspector_info['name']inspector_nameDirect mappinginspector_info['phone']inspector_phoneDirect mappingreport_recipientreport_recipientSingle field with delivery method includedviolations counttotal_violationsCalculatedMajor violations countmajor_violationsCalculatedpdf_filenamepdf_filenameDirect mappingPDF file pathpdf_pathFile system pathreport_notesreport_notesDirect mappinginspector_info['accepted_by']co_inspectorMapping correctedcompliance_data['reinspection_required']reinspection_requiredBooleancompliance_data['closure_suspension_required']closure_suspension_requiredBooleanbarcode_databarcode_dataDirect mappingequipment['pool_data']['pool_capacity_gallons']pool_capacity_gallonsDirect mappingequipment['pool_data']['pool_flow_rate_gpm']pool_flow_rate_gpmDirect mappingwater_chemistrywater_chemistry_detailsJSON string
violations table:
PDF Extraction FieldDatabase FieldMapping NotesReport IDreport_idForeign keyFacility IDfacility_idForeign keyviolation_codeviolation_codeDirect mappingviolation_titleviolation_titleDirect mappingobservationsobservationsDirect mappingcorrective_actioncorrective_actionNote: singular formcode_descriptioncode_descriptionDirect mappingis_major_violationis_major_violationBooleanseverity_levelseverity_levelNote: integer, not severity_score
equipment table:
PDF Extraction FieldDatabase FieldMapping NotesReport IDreport_idForeign keyFacility IDfacility_idForeign keyequipment['pool_data']['pool_capacity_gallons']pool_capacity_gallonsDirect mappingequipment['pool_data']['pool_flow_rate_gpm']flow_rate_gpmDirect mappingEquipment pieces detailsVarious equipment fieldsParsed from equipment_pieces array
...
Clear Separation of Concerns: Each JS module has single
RetryBContinueEditbash# Continue the milestone update
cat >> /tmp/milestone_20250822_updated.md << 'EOF'
...
### **PDF Extraction Development Notes**
- **Field Alignment**: All extraction fields must align exactly with database schema
- **Date Handling**: Always normalize dates to YYYY-MM-DD format before database save
- **Error Handling**: Use ErrorHandler for consistent logging throughout extraction
- **Violation Parsing**: Prevent cross-contamination between violation descriptions
- **Equipment Data**: Save to equipment table, not inspection_reports embedded fields
- **Water Chemistry**: Store as JSON string in water_chemistry_details field

### **Database Development Notes**
- **Two-Database Architecture**: Use database_config.py for proper database routing
- **Foreign Keys**: Always maintain referential integrity between tables
- **Field Names**: Follow exact database schema field names (corrective_action not corrective_actions, severity_level not severity_score)
- **Data Types**: Respect field types (severity_level as integer, booleans for compliance flags)

### **Service Development Notes**
- **Error Handling**: All services must use ErrorHandler for consistent logging
- **Database Connections**: Use DatabaseConfig for proper connection management
- **Atomic Operations**: Ensure transactional consistency for multi-table operations
- **Health Monitoring**: Include health checks in all external system interactions

### **Frontend Development Notes**
- **Module Pattern**: Each JS module should have single responsibility
- **Service Layer**: Abstract all API communication through service classes
- **State Management**: Use PageStateService for persistent state
- **Error Display**: Provide user-friendly error messages with actionable guidance
